<img src="https://github.com/Arxsher/Arxsher/raw/master/hero.png"></img>

## 👋 Hi there, I'm Arsher.

```
(lldb) po [self description]
{
    💻 = "I'm a Full-stack developer in progress, focused on real-world web apps";
    🤔 = "I study UItechniques and everything related to user interface";
    🌟 = "I love crafting delightful user experience and wonderful things";
    🔨 = "I use React, Laravel, TypeScript, Python, APIs, etc.";
    🐧 = "I daily drive Arch Linux + Hyprland, terminal-first workflow";
    💬 = "I'm active on Twitter (@Arxsher), DMs are welcomed";
}
```
